...
def _wait_for_kafka(bootstrap: str = "kafka:9092", retries: int = 30, delay: int = 5):
    """Block until the Kafka broker is reachable and ready to serve requests.

    Uses kafka-python's KafkaAdminClient (already a requirements.txt dependency)
    to issue a real broker API call rather than a bare TCP connect.  A bare TCP
    connect passes the moment the socket opens – which is before the broker has
    elected a leader for __consumer_offsets.  Calling list_topics() forces the
    full broker handshake and confirms the coordinator is operational.
    """
    import time
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            return
        except (NoBrokersAvailable, KafkaConnectionError, Exception) as exc:
            print(f"⏳ Kafka not ready yet (attempt {attempt}/{retries}): {exc}")
            time.sleep(delay)

    raise RuntimeError(f"Kafka at {bootstrap} did not become ready after {retries * delay}s")
...
with DAG(
    ...
    kafka_ready = PythonOperator(
        task_id="wait_for_kafka",
        python_callable=_wait_for_kafka,
        op_kwargs={"bootstrap": "kafka:9092", "retries": 30, "delay": 5},
    )
    ...
    t0 >> kafka_ready >> t1 >> spark_kafka_bronze >> spark_silver >> spark_gold
